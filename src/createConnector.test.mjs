...
test('createConnector, stream outgoing end with close trigger error', async () => {
  ...
  server.on('close', () => {
    assert.equal(onClose.mock.calls.length, 0);
    assert.equal(onError.mock.calls.length, 1);
    assert.equal(onError.mock.calls[0].arguments[0].message, 'Socket closed unexpectedly');
  });
  let i = 0;
  let isPause = false;
  const onDrain = mock.fn(() => {
...
test('createConnector socket emit error', async () => {
  ...
  const onConnect = mock.fn(() => {});
  const onError = mock.fn((error) => {
    assert.equal(error.message, 'aaaaxxxxxx');
    assert(!socket.eventNames().includes('connect'));
  });
  const onData = mock.fn(() => {
  });
  ...
  process.nextTick(() => {
    assert(socket.eventNames().includes('error'));
    assert(socket.eventNames().includes('connect'));
    socket.emit('error', new Error('aaaaxxxxxx'));
    assert(socket.eventNames().includes('error'));
    assert(!socket.eventNames().includes('connect'));
    socket.emit('error', new Error('cccc'));
    socket.emit('error', new Error('eee'));
  });
  await waitFor(200);
  assert.equal(onError.mock.calls.length, 1);
  assert.equal(onClose.mock.calls.length, 0);
...
test('createConnector end before connect', async () => {
  ...
  const onClose = mock.fn(() => {});
  const onError = mock.fn((error) => {
    assert.equal(error.message, 'socket not connect');
  });
  const connector = createConnector(
    {
      onData,
...
test('createConnector end signal abort' , async () => {
  ...
  if (onError.mock.calls.length === 1) {
    assert(s.includes(Buffer.concat(handleDataOnSocket.mock.calls.map((d) => d.arguments[0])).toString()));
  } else {
    assert.equal(
      Buffer.concat(handleDataOnSocket.mock.calls.map((d) => d.arguments[0])).toString(),
      s,
    );
  }
...
test('createConnector detach 3', async () => {
  const port = getPort();
  const server = net.createServer(() => {
  });
  server.listen(port);
  ...
  const ret = connector.detach();
  assert.equal(ret, null);
  assert(socket.destroyed);
  socket.emit('error', new Error('aaa'));
  await waitFor(300);
  assert.equal(onError.mock.calls.length, 0);
  server.close();
});
